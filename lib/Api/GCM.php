...
class GCM {
    ...
    public function __construct() {
        ...
        /*$this->apiKey = \Cerceau\System\Registry::instance()->SpecialConfig()->get('gcm_api_key');
        $this->apiSendAddress = \Cerceau\System\Registry::instance()->SpecialConfig()->get('gcm_api_send_address');*/
        $this->apiKey = 'AIzaSyBg6OyUX3Qr9UFWv-gVHorDwyI0Tv3yeOo';
        $this->apiSendAddress = 'https://android.googleapis.com/gcm/send';

        if (!$this->apiKey) {
            \Cerceau\System\Registry::instance()->Logger()->log( 'GCM', 'GCM: Needed API Key' );
        }

        if (!$this->apiSendAddress) {
            \Cerceau\System\Registry::instance()->Logger()->log( 'GCM', 'GCM: Needed API Send Address' );
        }
    }
